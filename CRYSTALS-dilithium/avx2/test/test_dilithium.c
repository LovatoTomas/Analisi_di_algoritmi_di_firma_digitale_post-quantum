...
Description: performance test per Dilithium, 
              rimosso la misurazione del tempo per la firma non corretta (messaggio corrotto).
              rimosso la memorizzazione dei cicli CPU.
*/
...
int main(int argc, char **argv)
{
  ...
  for(MLEN = 64; MLEN < MAXMLEN; MLEN = MLEN * INCREMENT) 
  {
    ...
    sprintf(output_line, "|%lu|%lu|%d|%d|%d|%f|%f|%f|\r\n", 
      ...
      average_double(ok_check_time, ITERATIONS));
