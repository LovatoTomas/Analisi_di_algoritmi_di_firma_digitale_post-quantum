...
Description: performance test per Falcon sul messaggio puro
*/
...
static int bench_sign_dyn(void *ctx, unsigned long num)
{
	...
	while (num -- > 0) {
		...
		CC(falcon_sign_dyn(&bc->rng,
			...
			bc->sk, FALCON_PRIVKEY_SIZE(bc->logn),
			bc->msg, bc->msg_len, bc->tmp, bc->tmp_len)); // Aggiunto il riferimento all'hash messaggio variabile
	}
	return 0;
}
...
static void test_speed_falcon(unsigned logn, double threshold, size_t msg_len)
{
	...
	sprintf(output_line, "|%zu|%zu|%u|%u|%zu|%f|%f|%f|%zu|\r\n",
		bc.msg_len, bc.msg_len + bc.sig_len, FALCON_PUBKEY_SIZE(bc.logn),
		FALCON_PRIVKEY_SIZE(bc.logn), bc.sig_len, kg, sdc, vvc, bc.msg_len);
...
main(int argc, char *argv[])
{
	...
	const char *filename1 = "out_falcon2_sha256";
	const char *filename2 = "out_falcon5_sha256";
	if(argc > 2)
	{
		filename1 = argv[1];
